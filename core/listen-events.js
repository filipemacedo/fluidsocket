...
const processMethods = socketIO => ({ methods, name, method }) => {
	...
	return client
			.on(`${event}`, async (data) => {
				 ...
				 */
				try {
					const result = await action({ body: JSON.parse(data), ...(client || {}) })

					/**
				 	* 
				 	*/
					return after
							.reduce(async (p, c) => await c(socketIO)(result), result)
				}
				catch(e) {
...
const processListeners = socketIO => ({ listeners, name }) => {
	...
	return Object
			.keys(listeners)
			.map(method => processMethodsWithIO({ 
...
module.exports = socketIO => configs => {
	 ...
	 * @return {[type]}      [description]
	 */
	return names.reduce((p, name) => p.concat(processListenersWithIO({ 
